...
--   pnpm db:migrate
--
-- The statement-breakpoint markers below (arrow + "statement-breakpoint" — not
-- spelled out here because drizzle-kit splits on that literal string even
-- inside this comment!) are how drizzle-kit splits a custom migration into
-- separate statements (each is sent as its own query, which the extended
-- protocol requires). They are ordinary SQL line comments, so this file also
-- runs as-is via `psql -f db/policies.sql`.
--
