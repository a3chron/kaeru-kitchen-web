...
function getClient(): SupabaseClient {
  ...
  if (!supabaseUrl || !secretKey) {
    ...
    throw new Error(
      "Supabase server client is not configured (NEXT_PUBLIC_SUPABASE_URL / SUPABASE_SECRET_KEY).",
    );
  }
...
/**
 * Server-only Supabase client using the privileged secret key (`sb_secret_...`,
 * the new-style replacement for the legacy JWT service_role key; both bypass
 * RLS). It must NEVER be imported into a client component — the `server-only`
